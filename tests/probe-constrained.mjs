// Constrained-browser probe for 16bedlimit.com (Pattern 43, /ship silent-outcome gate,
...
// Public surface: the 'Ask' chat drawer (#ask-fab -> #ask-form). POST /api/chat is
// intercepted; the drawer must then say so in its newest assistant bubble in #ask-log.
...
const ctx = await browser.newContext({
  ...
  userAgent:
    'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0 Safari/537.36 16bedlimit-probe',
});
...
/** Assistant bubbles: the intro one is always there, so a tap must ADD one. */
const replies = () =>
  page.evaluate(() =>
    [...document.querySelectorAll('#ask-log .msg--a')].map((m) =>
      m.textContent.trim().slice(0, 200),
    ),
  );
/** The bubble a tap added, or null when the count did not grow. */
const added = async (n) => {
...
await tap(
  'home-open-ask',
  () => page.click('#ask-fab', { timeout: 5000 }),
  async () => ({ drawerOpen: await page.evaluate(() => !document.getElementById('ask').hidden) }),
);

const n1 = (await replies()).length;
await page.fill('#ask-in', 'What is the IMD exclusion?');
await tap(
  'home-ask-send-click',
  () => page.click('#ask-send', { timeout: 5000 }),
  async () => ({ reply: await added(n1) }),
);

const n2 = (await replies()).length;
await page.fill('#ask-in', 'Which states have a waiver?');
await tap(
  'home-ask-send-enter',
  () => page.press('#ask-in', 'Enter'),
  async () => ({ reply: await added(n2) }),
);
// ---------------------------------------------------------------- report
